...
/- English proof (it's short!): By subtracting n, m,
and k from both sides, we get 0 = 0, which is true
by reflexive equality.
-/
...
begin
  unfold multiple_of,
  assume h,
  cases h with nh hk_1,
  cases nh with w Pw,
  cases hk_1 with ww Pww,
  apply exists.intro (w * ww),
  rw Pw,
  rw Pww,
  ring,
end
...
example
  ...
  :=
begin
  cases SomeoneKnowsLogic with p kl,
...
example
  ...
  :=
begin
  assume h_exists h_all,
  cases h_exists with w Pw,
  let contra := h_all w,
  contradiction,
end
...
begin
  split,
  assume h_all h_exists,
  cases h_exists with a Pa,
  let contra := h_all a,
  contradiction,
...
example
  ...
  :=
begin
  assume h_exists t np,
  let contra := h_exists (exists.intro t np),
  contradiction,
end
...
begin
  ...
  cases h with a h,
  cases h with l r,
  apply or.inl (exists.intro a l),
  apply or.inr (exists.intro a r),
end